...
## Boon-systeem (M24, issue #411 → gebouwd in issue #414)

Aanvulling op het herhalende drie-stichtingsmomenten-patroon hierboven (Deel 2): een beloning voor de speler die een stad daadwerkelijk tot "groot" uitbouwt vóórdat hij verder trekt, in plaats van hem alleen af te straffen als hij te lang blijft hangen (het afstandsverval, Deel 1).
...
- Volgorde: eerst de bestaande `StichtingsMomentPopup` ("stad gesticht"), daarna — alleen als de voorwaarde vervuld is — een nieuwe Boon-pop-up met naam en uitleg.
- De inhoud van de individuele Boons (welke bonussen, hoeveel) komt in aparte, losse vervolgissues — dit blijft uitsluitend het framework (opslag, trekking, timing, pop-up); zie hieronder.

**Open ontwerpvragen — beantwoord in issue #414**:
1. **Trekking zonder terugleggen**: een al-bezeten Boon-id wordt uit de pool gefilterd voor een volgende trekking (`trekBoon`, `src/game/boons.ts`) — dezelfde Boon wordt dus nooit twee keer toegekend.
2. **Boons overleven een stad-ineenstorting**: ze zijn run-breed opgeslagen (`GameState.boons`, niet op `City` zoals het nooit-uitgebouwde `Relic`-concept) en gaan niet meer weg zolang de run zelf doorgaat.
...
**Implementatiestatus**: framework gebouwd (`src/game/boons.ts`: `BOON_POOL`/`trekBoon`/`komtInAanmerkingVoorBoon`/`sluitBoonMelding`, gehaakt in `stichtStad` (acties.ts), `GameState.boons`/`boonToegekendEvent` (types.ts, met save-migratie in save.ts), pop-up via het herbruikbare `GoudOntdektPopup`-component). De pool is sindsdien uitgebreid met eigen mechanische effecten: "Voorraadschuur van de Voorvaderen" (issue #428, `pasBoonEffectToe`): +15 opslagcap bovenop andere opslag-verhogende bronnen, plus meteen +15 van elke gedeelde-opslag-grondstof (hout/steen/erts/goud). "Oude Handelsroute" (issue #431): elke 5 beurten +1 wampum, zolang de speler de Boon bezit (`verwerkOudeHandelsrouteBoon`). "Zegeningen van het Moederland" (issue #540): de speler kiest bij toekenning een moederland (Ierland/Duitsland/Engeland/Italië, `kiesMoederland`), dat daarna elke 10 beurten een lading voedsel/hout/erts/steen levert (`verwerkZegeningenVanHetMoederlandBoon`).

Vierde Boon, "Trail Blazer" (issue #539, herzien in dezelfde issue van een ongelimiteerde "hele volgende streek in"-vorm naar de huidige puntenvorm): een blijvende, puntengedreven uitzondering op de kernregel dat de settler binnen al ontgrendeld gebied blijft (hoofdstuk 16 van het hoofddocument, `magSettlerNaar`/`bereikbarePosities` in `src/game/wegen.ts`). Zolang de speler deze Boon bezit, mag de settler ook individuele vakjes buiten al ontgrendeld gebied in lopen: elk zo'n vakje kost 1 trailblazer-punt en blijft daarna blijvend begaanbaar (`Tile.trailOntdekt`, types.ts) — geen streek-brede ontgrendeling en geen van de eenmalige streek-ontdekkingsevents (Goudader, stichtingskans, enz.): "de trailblazer ontdekt individuele vakjes en geen streken, de huidige streek-logica blijft ongewijzigd" (issue-discussie #539). De speler krijgt elke 20 beurten 5 nieuwe punten (`TRAIL_BLAZER_PUNTEN_INTERVAL_BEURTEN`/`TRAIL_BLAZER_PUNTEN_PER_INTERVAL`, `verwerkTrailBlazerPunten`, boons.ts, aangeroepen vanuit `volgendeBeurt`, economie.ts) — die rem voorkomt dat de speler in één keer de hele kaart verkent (expliciet de reden voor de herziening naar de puntenvorm). Een trailblazer-vakje is, zolang de streek zelf vergrendeld blijft, niet bebouwbaar met land improvements (`startBouw`, infrastructuurEnBouw.ts, filtert al op `Streek.ontgrendeld` — geen extra code nodig), maar de settler mag er wel een weg op aanleggen (`legWegAan`, acties.ts, controleert nooit op `ontgrendeld`) — op expliciet verzoek. De twee bevroren speciale streken (Bezette Streek, Wampanoag-laag, Deel 2 hierboven resp. hoofdstuk 9) blijven uitgesloten, ook met deze Boon — die lossen uitsluitend via hun eigen verhaallijn op (`magSettlerNaar`/`ontdekVakjeViaTrailBlazer`, `src/game/wegen.ts`, gebaseerd op de vaste hoogte/`campagneId` van die twee streken, niet op hun runtime-vlag — anders zou een settler die ver vóór de frontier uit loopt zo'n nog niet geïnitialiseerde streek gewoon voorbij kunnen lopen).
