...
Vierde Boon, "Baanbreker" (issue #539): geen grondstofeffect, maar een blijvende uitzondering op de kernregel dat de settler binnen al ontgrendeld gebied blijft (hoofdstuk 16 van het hoofddocument, `magSettlerNaar`/`bereikbarePosities` in `src/game/wegen.ts`) — met deze Boon mag de settler ook de eerstvolgende, nog vergrendelde streek in lopen. Elk vakje waar hij zo overheen loopt is vanaf dan gewoon ontdekt en begaanbaar: geen tijdelijke staat, geen terugkeer-eis, geen risico (issue-discussie #539: expliciete keuze van de opdrachtgever, in afwijking van de anders bewust uitgestelde "vooruitkijk verder dan 1 streek", CLAUDE.md). De twee bevroren speciale streken (Bezette Streek, Wampanoag-laag, Deel 2 hierboven resp. hoofdstuk 9) blijven uitgesloten — die lossen uitsluitend via hun eigen verhaallijn op, ook met deze Boon (`magBaanbrekerNaarStreek`, `src/game/streekOntgrendeling.ts`). De daadwerkelijke ontdekking (`ontdekStreekViaBaanbreker`, aangeroepen vanuit `verplaatsSettlerNaar` in acties.ts) hergebruikt dezelfde per-streek-ontgrendelingsstap als de normale, cultuur-gedreven `verwerkStreekOntgrendeling` (uit die functie losgetrokken in een gedeelde `ontgrendelEenStreek`-helper) — zodat een via Baanbreker overgeslagen streek toch nog al zijn eenmalige ontdekkings-events krijgt (Goudader, stichtingskans, rivier-aankondiging, Lakota-scout, de gegarandeerde roofdier-kudde), ook als de cultuur er later alsnog overheen groeit.
