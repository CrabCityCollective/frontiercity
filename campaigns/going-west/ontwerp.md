...
**Implementatiestatus**: framework gebouwd (`src/game/boons.ts`: `BOON_POOL`/`trekBoon`/`komtInAanmerkingVoorBoon`/`sluitBoonMelding`, gehaakt in `stichtStad` (acties.ts), `GameState.boons`/`boonToegekendEvent` (types.ts, met save-migratie in save.ts), pop-up via het herbruikbare `GoudOntdektPopup`-component). De pool bevat sinds issue #428 de eerste Boon mét mechanisch effect, "Voorraadschuur van de Voorvaderen" (`pasBoonEffectToe`, boons.ts): +15 opslagcap bovenop andere opslag-verhogende bronnen, plus meteen +15 van elke gedeelde-opslag-grondstof (hout/steen/erts/goud). Zolang dit het enige lid van de pool is, krijgt de speler 'm effectief altijd bij de eerste keer dat hij in aanmerking komt — bewust (issue #428), niet een bug: verdere Boons met eigen effecten volgen in losse vervolgissues.
