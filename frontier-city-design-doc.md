...
De in M20d gebouwde spel-opstart-architectuur (campagne-bewuste `GameState`/save-sleutel/campagnekeuze-doorgifte/placeholder-tegelset, hoofdstuk 13) blijft ongewijzigd en werkt. De laatste stap — Going West op `beschikbaar: true` zetten in `CampagneSelectScherm.tsx` — blijft geblokkeerd: blocker 1 heeft nu placeholder-content (M20f) in plaats van tutorial-lekkage, maar de intro-/oceaan-uitleg-tekst is nog niet meegenomen en de drie Anker-verhalen (issue #278, vraag 3) staan nog open.
