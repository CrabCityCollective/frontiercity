...
**Nu**: de Amerikaanse frontier-campagne inhoudelijk en technisch volledig uitwerken (ankers staan al beschreven in hoofdstuk 9; de meerdere-steden-fundering, het afstandsverval en de kern van het herhalende stichtingspatroon staan al in de code, M16-M18 — zie hoofdstuk 9/13). De `CampaignConfig`-instantie en de weergavenamen (M20a) staan inmiddels ook in de code, en sinds M20b ook een eigen, langere, vaste (handgeschreven, geen procedurele generatie — issue #247, hoofdstuk 8/11) campagnekaart (`src/game/worldGoingWest.ts`, 35 streken). Nog te doen: de koppeling van die kaart aan het daadwerkelijk starten/spelen van een Going West-run en aan `gegarandeerdeStichtingskansHoogten()` zodat het stichtingspatroon daadwerkelijk speelbaar wordt (M20c), de Anker-verhalen zelf, en resterende content zoals zeldzaamheid en pushback-diplomatie die in de tutorial-MVP bewust zijn uitgesteld. De tutorial blijft intussen gewoon bestaan en speelbaar als losstaande, eerste campagne (hoofdstuk 10) — dit is een verschuiving van bouwprioriteit, geen vervanging.
