...
| M20g | Going West — `beschikbaar: true` (issue #292, blocker 1 afronding) | Laatste twee openstaande stukken van blocker 1 (hoofdstuk 19): `introContentVoorCampagne()`/`oceaanUitlegVoorCampagne()` (`campagnes.ts`, zelfde val-terug-op-tutorial-dispatch-patroon als `streekContentVoorCampagne()`, M20f) geven `IntroScherm`/`OceaanUitlegPopup` nu campagne-bewuste titel/subtitel/tekst mee (`campagneId`-prop erbij, doorgegeven vanuit `GameRoot.tsx`) in plaats van de rechtstreekse `tutorialContent`-import. Going West krijgt een eigen titel ("Going West"/"American Expansion", hergebruikt uit `CampagneSelectScherm`) en een eigen oceaan-uitleg-titel ("De kust van de Stille Oceaan", passend bij de afdaling-naar-de-kust op streek 28-35 van de kaart, `worldGoingWest.ts`); de flavor-/uitleg-tekst zelf blijft "todo", zelfde bewuste placeholder-aanpak als M20f (issue #278, vraag 1). Met dit laatste stuk opgelost staat Going West nu op `beschikbaar: true` in `CampagneSelectScherm.tsx`. **Nog niet gedaan** (bewust geen poort voor `beschikbaar: true`, hoofdstuk 15/19): de drie Anker-verhalen (issue #278, vraag 3) en de overige, bewust uitgestelde content (zeldzaamheid, vooruitkijk, culturele pushback-diplomatie, CLAUDE.md) | Klaar |
...
**Nu**: de Amerikaanse frontier-campagne inhoudelijk en technisch volledig uitwerken (ankers staan al beschreven in hoofdstuk 9; de meerdere-steden-fundering, het afstandsverval en de kern van het herhalende stichtingspatroon staan al in de code, M16-M18 — zie hoofdstuk 9/13). De `CampaignConfig`-instantie en de weergavenamen (M20a) staan inmiddels ook in de code, sinds M20b ook een eigen, langere, vaste (handgeschreven, geen procedurele generatie — issue #247, hoofdstuk 8/11) campagnekaart (`src/game/worldGoingWest.ts`, 35 streken), sinds M20c is de vers-water-dichtheid van die kaart geverifieerd tegen `gegarandeerdeStichtingskansHoogten()` (`vindStichtingskansGaten()`, stad.ts), sinds M20d is de spel-opstart-architectuur zelf campagne-bewust (`GameState.campagneId`, per-campagne save-sleutel, campagnekeuze-doorgifte, placeholder-tegelset-koppeling — hoofdstuk 13), en sinds M20g staat Going West op `beschikbaar: true` in `CampagneSelectScherm.tsx` — alle drie blockers uit hoofdstuk 19 zijn inmiddels opgelost, de laatste twee (intro-/oceaan-uitleg-tekst) met dezelfde campagne-bewuste placeholder-aanpak als het per-streek-flavor-deel (M20f). Nog te doen, als los, doorlopend proces zonder vaste opleverdatum: de drie Anker-verhalen (hoofdstuk 9, issue #278 vraag 3) inhoudelijk en technisch uitwerken, en de placeholder-flavor-tekst (per streek en bij de intro-/oceaan-uitleg) gaandeweg vervangen door echte tekst — geen van beide is een poort voor `beschikbaar: true`, dat staat immers al aan. Resterende content zoals zeldzaamheid en culturele pushback-diplomatie blijft bewust uitgesteld (CLAUDE.md). De tutorial blijft intussen gewoon bestaan en speelbaar als losstaande, eerste campagne (hoofdstuk 10) — dit is een verschuiving van bouwprioriteit, geen vervanging.
