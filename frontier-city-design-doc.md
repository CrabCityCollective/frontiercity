...
- Meerdere steden/frontier-verplaatsing, inclusief het bijbehorende afstandsverval van city improvements en het herhalende drie-stichtingsmomenten-patroon (hoofdstuk 9/11/14, Amerikaanse campagne) — dat mechanisme is uitgewerkt/gedocumenteerd, en zowel de datamodel-fundering (`GameState.steden`/`City.streekHoogte`, M16 hierboven) als het effectiviteitsverval zelf (`stadEffectiviteit`, M17 hierboven) staan inmiddels in de code. Het herhalende stichtingspatroon/de tweede-stad-flow (Deel 2, M18) is nog niet gebouwd, dus een run kan nog altijd niet meer dan 1 stad hebben — en `stadEffectiviteit` geeft dan altijd 100% terug (hoofdstuk 11), waardoor de speler nog geen ander gedrag ziet dan vóór M16/M17
