...
- Een daadwerkelijk speelbare Amerikaanse frontier-campagne, inclusief het herhalende drie-stichtingsmomenten-patroon in de praktijk (hoofdstuk 9/11/14) — het onderliggende mechanisme (datamodel-fundering `GameState.steden`/`City.streekHoogte` (M16), effectiviteitsverval `stadEffectiviteit` (M17), en de `stichtStad`-herschrijving + `gegarandeerdeStichtingskansHoogten()` (Deel 2, M18)) staat inmiddels in de code, maar de tutorial (vast, 14 streken, precies één vers-water-vakje) kan het nooit meer dan één keer laten zien. Wat nog ontbreekt: een echte, langere campagnewereld (30-60 streken) met een eigen `CampaignConfig`-instantie en worldgen die de drie kansen per cyclus daadwerkelijk plaatst — pas dan wordt het patroon voor een speler voelbaar (zie hoofdstuk 15, voorgestelde M20)
...
| M20 | Amerikaanse-campagne wereld-fundering | **Voorgesteld, nog niet gebouwd.** Nu de Amerikaanse frontier-campagne het actieve bouwdoel is (hoofdstuk 15), is dit de logische vervolgstap op M16-M18: een echte `CampaignConfig`-instantie voor deze campagne (de weergavenamen-tabel in hoofdstuk 9 ligt al klaar), een langere/geschaalde streek-generatie (30-60 streken, hoofdstuk 8/14, i.p.v. de vaste 14-streek tutorial-wereld) en het daadwerkelijk koppelen van `gegarandeerdeStichtingskansHoogten()` (M18) aan die worldgen zodat de drie stichtingskansen per cyclus (hoofdstuk 9 Deel 2) voor het eerst echt speelbaar worden. Bewust apart van M18 gehouden — dit raakt de worldgen-architectuur (nu een vaste, handgeschreven tabel per streek-hoogte, zie `world.ts`) en is te groot/onzeker om blind in één sessie mee te nemen met de reducer-wijziging | Nog te doen |
