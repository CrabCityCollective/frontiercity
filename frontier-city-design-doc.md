...
- **Win-conditie**: bereik de oceaan aan de overkant van een vast, handgeschreven continent (per campagne een eigen kaart, geen procedurele generatie — issue #247, hoofdstuk 8).
...
**Bezette Streek & Confrontatie** (issue: "De Bezette Streek, missionaris en verkenner" — vervangt de eerdere, kleinere "militaire confrontatie op streek 12"-placeholder volledig; herzien door issue "Bezette streek scherm" naar een klikbare-kaart-flow, zie hoofdstuk 11 voor de volledige onderbouwing; hoofdstuk 10 voor de tutorial-scripting op streek 12, hoofdstuk 14 voor alle tunbare getallen):
...
- **Worldgen** (hoogte/lengte van continent, terrein, obstakels) is **vast**: elke campagne heeft één eigen, volledig handgeschreven kaart, precies zoals de tutorial (hoofdstuk 10/11/13) — geen procedurele generatie. **Besloten na issue #247**: dit verving de eerdere aanpak ("worldgen blijft random per run — dat blijft het ontdekkingsgevoel"); zie hoofdstuk 11 ("Vaste, handgeschreven campagnekaarten i.p.v. procedurele worldgen") voor de volledige afweging.
...
**Implementatiestatus (M16-M18, zie hoofdstuk 13)**: de datamodel-fundering (`GameState.steden`/`City.streekHoogte`), het effectiviteitsverval (`stadEffectiviteit`) en de kern van Deel 2 staan inmiddels in de code. `stichtStad` (acties.ts) voegt bij een stichting een echte nieuwe stad toe aan `steden` in plaats van de bestaande te vervangen, en maakt haar de actieve stad; het einde van de run (`stadGesticht`) wordt alleen nog getriggerd door de allerlaatste, verplichte stichting aan het einde van de wereld — elke andere stichting laat de run gewoon doorlopen. De drie kans-hoogten uit Deel 2 hierboven zijn vastgelegd als een pure functie, `gegarandeerdeStichtingskansHoogten()` (stad.ts): +3/+8/+13 streken t.o.v. de streekHoogte van de stichtende stad (vaste MVP-keuzes binnen de 0-4/5-12/13+-vensters, geen willekeur). **Nog niet gedaan**: deze functie is nog nergens aan de daadwerkelijke worldgen gekoppeld — de tutorial-wereld is vast en precies 14 streken lang met precies één vers-water-vakje (op de allerlaatste streek), dus het patroon kan daar per definitie nooit meer dan die ene, afsluitende stichting laten zien. Om het herhalende patroon daadwerkelijk speelbaar te maken is een eigen, langere, vaste (handgeschreven) Amerikaanse-campagne-wereld nodig (zie hoofdstuk 15).
...
**Missionaris-vereiste voor de belegeringsmeter (herzien door issue: "Bezette streek scherm" — zie "Klikbare kaart i.p.v. een los stadsscherm-paneel" hieronder voor de huidige, per-Heiligdom-vorm)**
...
**Meerdere Missionarissen vermenigvuldigen de belegeringsmeter-snelheid (issue: "laatste confrontatie tweaken"; het principe blijft overeind na "Bezette streek scherm", nu per-Heiligdom i.p.v. streek-breed — zie hieronder)**
...
**Een harde Wachttoren-eis voor een Confrontatie tegen een Bezette Streek, met een verlies-effect dat de bemanning kost maar niet de Wachttoren zelf (issue: "laatste confrontatie tweaken"; de harde eis zelf is vervangen door een Legerkamp-eis via issue "Bezette streek scherm" — zie hieronder; het lichtere verlies-effect blijft hetzelfde principe, nu gericht op een Legerkamp-strijder)**
...
**Klikbare kaart i.p.v. een los stadsscherm-paneel (issue: "Bezette streek scherm")**
De oorspronkelijke Bezette-Streek-UI bundelde Verkenner-rekrutering, Verkenning-modus, Missionaris-rekrutering, de belegeringsmeter en de Confrontatie-doelenlijst allemaal in één paneel binnen het stadsmenu — hetzelfde scherm waar de speler groei, opslag, stadsverbeteringen en het militaire paneel beheert. Dat vermengde twee verschillende soorten interactie: stadsbeheer (dingen die niets met een specifieke plek te maken hebben) en streek-specifieke acties (die per definitie over een concreet vakje gaan). Het patroon van een Wachttoren bemannen — eerst op de tile zelf klikken, dan pas de keuze maken — bestond al sinds issue "wachttorens bemannen" en bleek precies zo goed te werken voor Verkenning (klik een verhuld vakje), Confrontatie (klik de vijandelijke Wachttoren) en Missionaris-toewijzing (klik het vijandelijke Heiligdom): de tile-info-pop-up krijgt per geval de relevante actie erbij (`verkenningVraag`/`confrontatieVraag`/`missionarisVraag`), precies zoals `wachttorenVraag` dat al deed. Het overgebleven statusbalkje op de kaart (buiten het stadsmenu) toont alleen nog samenvattende voortgang (lopende verkenningen, wololo-meters) — geen acties meer, puur zichtbaarheid.

**Verkenner niet langer een vooraf te trainen eenheid (issue: "Bezette streek scherm")**
De oorspronkelijke Verkenner-eenheid moest eerst getraind worden (een aparte wachtrij, `City.verkennerInAanbouw`) voordat de Verkenning-actie ooit beschikbaar kwam — een extra stap die weinig toevoegde, omdat de eenheid zelf geen andere rol had dan "bestaat, dus Verkenning is mogelijk" (geen positie, geen individuele toewijzing). Door het sturen van een verkenner rechtstreeks aan de klik op het doelvakje te koppelen — met dezelfde grondstofkosten en dezelfde tijdsduur als de vroegere eenheid, nu als een per-vakje aftellend tellertje (`Tile.verkenningInGang`) — verdwijnt die tussenstap zonder de onderliggende afweging te verliezen: de speler betaalt nog steeds grondstoffen en wetenschap, en wacht nog steeds een aantal beurten voordat het vakje onthuld wordt. De 1×-per-beurt-limiet (hoofdstuk 6) blijft bestaan als rem op hoe snel de hele streek verkend kan worden, nu losgekoppeld van hoeveel verkenningen tegelijk "onderweg" mogen zijn.

**Missionaris-toewijzing per Heiligdom i.p.v. een streek-brede belegeringsmeter, en veroveren i.p.v. vernietigen (issue: "Bezette streek scherm")**
De eerdere versie liet élke bestaande Missionaris meetellen voor één gedeelde meter tegen het eerst-onthulde vijandelijke Heiligdom, gevoed door cultuurproductie die toch al nergens anders heen kon. Dat paste niet meer bij een klikbare kaart, waar de speler een Missionaris juist naar een concreet vakje stuurt — een acties-op-de-kaart-model vraagt om een doel-op-de-kaart, niet om een abstract "er bestaat een Missionaris ergens". Elk vijandelijk Heiligdom kreeg daarom een eigen wololo-meter, gevuld door de Missionarissen die er specifiek naartoe gestuurd zijn (met een vaste opbrengst per beurt per Missionaris, in plaats van omgeleide cultuurproductie — een streek-brede cultuurstroom bestaat immers niet meer als invoer zodra het doel per Heiligdom uiteenvalt). Het eindresultaat veranderde mee: een "belegerd" Heiligdom vernietigen paste bij een generieke meter zonder duidelijke eigenaar, maar bij een Missionaris die één specifiek Heiligdom bewerkt, past "bekeren" (het wordt eigen bezit, met cultuuropbrengst) thematisch beter dan "slopen" — vandaar de naam "wololo-meter". De toegewezen Missionarissen komen na verovering vrij voor een volgend doel, in plaats van simpelweg te blijven bestaan zonder nog iets te doen.

**Legerkamp i.p.v. Wachttoren als eis voor een Confrontatie tegen een Bezette Streek (issue: "Bezette streek scherm")**
Thematisch is een Wachttoren een verdedigingswerk (hij weert indringers af, hoofdstuk 6), terwijl een Legerkamp de eenheid is die zelf aanvalt — een Confrontatie starten vanaf een Wachttoren voelde daarom als de verkeerde infrastructuur voor de verkeerde actie. Het Legerkamp had toch al exact de rol die hier nodig is: een toegewezen Soldaat "marcheert zelf naar de Bezette Streek", ongeacht waar het Legerkamp staat (zie "Legerkamp overal bouwbaar" hieronder) — de eis simpelweg verplaatsen van "een bemande Wachttoren op de streek eronder" naar "een voltooid Legerkamp op de streek eronder" hergebruikt die bestaande rol in plaats van een nieuwe te verzinnen. Bemanning van dat Legerkamp is geen aparte eis (in tegenstelling tot de oude Wachttoren-eis) omdat de Legerkamp-legerwaarde toch al apart en overal meetelt (`berekenLegerkampLegerwaarde`) — de eis is puur "staat dit Legerkamp er en is het wegverbonden", niet "is het ook bemand". Het lichtere verlies-effect uit "laatste confrontatie tweaken" (hierboven) blijft intact, alleen het doelwit verschuift: bij verlies is niet langer de Wachttoren-bemanning het slachtoffer, maar één van de Legerkamp-toegewezen strijders (de eerste, bij gebrek aan een aparte "wie voerde het bevel"-notie).
...
**Vaste, handgeschreven campagnekaarten i.p.v. procedurele worldgen (issue #247)**
Hoofdstuk 8 stelde eerder dat worldgen (continent-lengte, terrein, obstakels) random per run zou blijven, als tegenwicht tegen de vaste tutorial-kaart — dat zou het "ontdekkingsgevoel" bewaken dat de tutorial, met zijn volledig handgeschreven streken, bewust niet had. Na de tutorial-MVP (die volledig op vaste, handgeschreven streek-data draait, hoofdstuk 10/11/13, en na "tig" keer spelen goed bleek te werken) is besloten om ditzelfde patroon door te trekken naar elke campagne, inclusief de Amerikaanse frontier-campagne: elke campagne krijgt één eigen, volledig handgeschreven kaart in plaats van een procedureel gegenereerde.
...
**Roofdieren**
- Vanaf **streek 6** (issue: "Eerste streek geen roofdieren" — eerder al eens verschoven naar streek 1 door issue "jagen en farmen omdraaien", wat in de praktijk te vroeg bleek: spelers verloren regelmatig een settler voordat ze de mechaniek ooit bewust hadden leren kennen) brengt jagen een risico met zich mee. Streek 6 introduceert verder geen andere nieuwe mechaniek, dus het roofdier krijgt hier de volle aandacht.
...
Bij het afronden van M20d (hoofdstuk 13, issue: "M20d voorstel") is de spel-opstart-architectuur zelf campagne-bewust gemaakt (`GameState.campagneId`, per-campagne save-sleutel, de `CampagneSelectScherm` → `AppRoot` → `GameRoot` → `useGameEngine`-campagnekeuze-doorgifte, en een placeholder-tegelset-koppeling in de renderer). Bij het volledig doorlezen van `GameRoot.tsx` om de laatste stap te zetten — Going West op `beschikbaar: true` — bleek dat de streek-flavor- en uitleg-pop-up-laag daar nog los van staat: die is nog vrijwel volledig tutorial-specifiek, en één van de gevonden problemen is een echte functionele bug, geen contentkeuze. Dit hoofdstuk legt de drie blockers vast als uitgangspunt voor een nog voor te stellen vervolg-milestone (CLAUDE.md: architectuur-/content-rakende bevindingen eerst apart voorstellen in plaats van ze blind in dezelfde sessie mee te nemen).

**1. Tutorial-narratieve content is niet campagne-bewust**
`tutorialContent.ts` (`TUTORIAL_STREEK_CONTENT`, `INTRO_TITEL`/`INTRO_FLAVOR_TEKST`, `TUTORIAL_VOLTOOID_TITEL`, `BEZETTE_STREEK_TITEL`/`_TEKST`, `STICHTING_AFSLUITENDE_SCENE`, e.a.) bevat expliciete Hertenpad-volk-/Oer-stad-lore (hoofdstuk 10) en kijkt nergens naar `state.campagneId`. Op `beschikbaar: true` zou een Going West-run dus letterlijk tutorial-teksten tonen ("De Bergkam is bezet", "De Eerste Vuren — voltooid", enz.) — inhoudelijk fout, geen technisch detail. Oplossing vergt een Going West-equivalent van dit bestand: minimaal eigen intro-/voltooid-tekst en per-streek-namen/flavor-tekst tot de laatste streek van de campagnekaart (35, hoofdstuk 13 M20b), of een generieke fallback zonder flavor-tekst voor streken zonder eigen content. Qua omvang vergelijkbaar met hoofdstuk 10 zelf — een eigen inhoudelijke schrijftaak, geen technische afronding meer.

**2. `TUTORIAL_STREEK_AANTAL` hard-coded voor de oceaan-uitleg**
`GameRoot.tsx` gebruikt de tutorial-constante `TUTORIAL_STREEK_AANTAL` (14, `world.ts`) om te bepalen wanneer de oceaan-uitleg-pop-up moet verschijnen. Bij `beschikbaar: true` zou dit op streek 14 van de 35-streken Going West-kaart (hoofdstuk 13 M20b) afgaan, niet aan het echte einde van die kaart. Vergt een per-campagne streek-aantal in plaats van de tutorial-constante.

**3. Streek-popup-softlock voorbij streek 14 (functionele bug, geen contentkeuze)**
`TUTORIAL_STREEK_CONTENT` kent alleen entries voor streek 1-14; `StreekPopup` rendert `null` zodra er voor de huidige streek geen content gevonden wordt. `laatstBevestigdeStreek` (`GameRoot.tsx`) wordt echter uitsluitend bijgewerkt via de "Doorgaan"-knop in diezelfde popup. Zodra een campagnekaart langer is dan 14 streken (Going West: 35) blijft `laatstBevestigdeStreek` dus permanent achter vanaf streek 15 — en via de bestaande `!toonStreekPopup`-gating-keten blokkeert dat daarmee alle lager-prioriteit pop-ups voor de rest van de run, inclusief de **verplichte** tech-keuze-pop-up (hoofdstuk 13 M13). Anders dan blocker 1/2 is dit geen inhoudelijke tutorial-koppeling maar een echte bug: hij treft elke toekomstige campagnekaart die langer is dan 14 streken, niet specifiek Going West. Fix: `toonStreekPopup` moet ook vereisen dat er daadwerkelijk streek-content bestaat voor de huidige streek, niet alleen dat de streek nog niet bevestigd is.

**Samenhang met het herhalende stichtingspatroon**
Los van bovenstaande drie, maar bij hetzelfde onderzoek gevonden: `TutorialVoltooidPopup` ("De Eerste Vuren — voltooid") zou bij élke stichting binnen het herhalende drie-stichtingsmomenten-patroon verschijnen (hoofdstuk 9 Deel 2, M18), niet alleen bij de laatste — in een Going West-run potentieel 3+ keer per run. Dit hoort bij dezelfde content-taak als blocker 1 (een eigen Going West-"voltooid"-signaal per stichting, of bewust weglaten voor de eerste versie). Ook het "voltooid"-vinkje in `CampagneSelectScherm.tsx` (`heeftTutorialVoltooid()`) is hier relevant: dat moet bij elke andere campagne dan de tutorial-entry zelf verborgen blijven — anders krijgt Going West (of elke andere latere campagne) per ongeluk hetzelfde vinkje zodra de tutorial ooit voltooid is, terwijl er nog geen eigen Going West-voltooid-signaal bestaat.

**Wat dit niet verandert**
De in M20d gebouwde spel-opstart-architectuur (campagne-bewuste `GameState`/save-sleutel/campagnekeuze-doorgifte/placeholder-tegelset, hoofdstuk 13) blijft ongewijzigd en werkt. Alleen de laatste stap — Going West op `beschikbaar: true` zetten in `CampagneSelectScherm.tsx` — blijft geblokkeerd totdat blocker 1 (incl. de samenhang met het stichtingspatroon hierboven) inhoudelijk is uitgeschreven en blockers 2/3 zijn opgelost.
