...
| M20a | Going West — `CampaignConfig` & weergavenamen | Een echte `CampaignConfig`-instantie voor de Amerikaanse frontier-campagne (id/naam/tegelSet/campagne-specifieke multipliers, hoofdstuk 8), de weergavenamen-tabel uit hoofdstuk 9 volledig ingevuld (`improvementNamen`/`techNamen`, types.ts), en `beschikbaar: true` in `CampagneSelectScherm.tsx` zodra er met M20b een speelbare kaart achter zit. Geen worldgen-wijziging — kleinste, niet-architectuur-rakende stap (issue #247) | Nog te doen |
| M20b | Going West — vaste campagnekaart | Een eigen, volledig handgeschreven campagnekaart voor de Amerikaanse frontier-campagne, zelfde patroon als de tutorial (`Record<hoogte, ...>`-tabellen voor terreintype/subtype/vers-water/amberader/dreigingsniveau in `world.ts`), losstaand van de bestaande tutorial-tabellen die ongewijzigd blijven. Lengte begint bij één vaste kaart op "Normaal"-lengte (30-40 streken, hoofdstuk 14); moeilijkheidsgraad-varianten (eigen kaarten voor Makkelijk/Moeilijk, hoofdstuk 14) volgen als latere uitbreiding, geen harde M20b-eis. **Besloten (issue #247, hoofdstuk 8/11)**: vervangt het eerder voorgestelde plan voor seeded-RNG/procedurele streek-generatie — te veel nieuwe, ongeteste architectuur bovenop een tutorial-patroon dat al bewezen werkt | Nog te doen |
| M20c | `gegarandeerdeStichtingskansHoogten()` koppelen aan de vaste kaart | Vers-water-vakjes dicht genoeg over de M20b-kaart verspreiden (richtlijn: interval ≤ 4 streken) dat de drie afstandszones uit `gegarandeerdeStichtingskansHoogten()` (M18: 0-4/5-12/13+ t.o.v. de streekHoogte van de laatst gestichte stad) altijd een treffer hebben — ongeacht op welke hoogte de speler daadwerkelijk sticht. Een vaste kaart kan de exacte, pas tijdens het spelen bekende stichtingshoogte niet vooraf raden (anders dan bij procedurele generatie, waar je ná de stichting op de berekende hoogte kunt plaatsen); de garantie wordt daarom via dichtheid van vooraf geplaatste vakjes gehaald in plaats van exacte, dynamische plaatsing. Zodra dit staat, wordt het herhalende drie-stichtingsmomenten-patroon (hoofdstuk 9 Deel 2) voor het eerst echt speelbaar | Nog te doen |
...
**Nu**: de Amerikaanse frontier-campagne inhoudelijk en technisch volledig uitwerken (ankers staan al beschreven in hoofdstuk 9; de meerdere-steden-fundering, het afstandsverval en de kern van het herhalende stichtingspatroon staan al in de code, M16-M18 — zie hoofdstuk 9/13). Nog te doen: een echte `CampaignConfig`-instantie en de weergavenamen (M20a), een eigen, langere, vaste (handgeschreven, geen procedurele generatie — issue #247, hoofdstuk 8/11) campagnekaart (M20b), de koppeling die het stichtingspatroon daadwerkelijk speelbaar maakt (M20c), de Anker-verhalen zelf, en resterende content zoals zeldzaamheid en pushback-diplomatie die in de tutorial-MVP bewust zijn uitgesteld. De tutorial blijft intussen gewoon bestaan en speelbaar als losstaande, eerste campagne (hoofdstuk 10) — dit is een verschuiving van bouwprioriteit, geen vervanging.
