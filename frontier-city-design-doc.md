...
**3. Streek-popup-softlock voorbij streek 14 (functionele bug, geen contentkeuze) (opgelost, M20e)**
`TUTORIAL_STREEK_CONTENT` kent alleen entries voor streek 1-14; `StreekPopup` rendert `null` zodra er voor de huidige streek geen content gevonden wordt. `laatstBevestigdeStreek` (`GameRoot.tsx`) wordt echter uitsluitend bijgewerkt via de "Doorgaan"-knop in diezelfde popup. Zodra een campagnekaart langer is dan 14 streken (Going West: 35) bleef `laatstBevestigdeStreek` dus permanent achter vanaf streek 15 — en via de bestaande `!toonStreekPopup`-gating-keten blokkeerde dat daarmee alle lager-prioriteit pop-ups voor de rest van de run, inclusief de **verplichte** tech-keuze-pop-up (hoofdstuk 13 M13). Anders dan blocker 1/2 was dit geen inhoudelijke tutorial-koppeling maar een echte bug: hij treft elke toekomstige campagnekaart die langer is dan 14 streken, niet specifiek Going West. Fix: `toonStreekPopup` vereist nu ook dat er daadwerkelijk streek-content bestaat voor de huidige streek, niet alleen dat de streek nog niet bevestigd is — zodra blocker 1 een Going West-equivalent van `streekContent()` toevoegt, moet die aanroep hier ook naar kijken (nu kijkt hij nog uitsluitend naar `TUTORIAL_STREEK_CONTENT`, wat voor Going West betekent dat er tot blocker 1 domweg geen streek-pop-up verschijnt, in plaats van een softlock).

**Samenhang met het herhalende stichtingspatroon**
Los van bovenstaande drie, maar bij hetzelfde onderzoek gevonden: `TutorialVoltooidPopup` ("De Eerste Vuren — voltooid") zou bij élke stichting binnen het herhalende drie-stichtingsmomenten-patroon verschijnen (hoofdstuk 9 Deel 2, M18), niet alleen bij de laatste — in een Going West-run potentieel 3+ keer per run. Dit hoort bij dezelfde content-taak als blocker 1 (een eigen Going West-"voltooid"-signaal per stichting, of bewust weglaten voor de eerste versie) — **nog open**. Het "voltooid"-vinkje in `CampagneSelectScherm.tsx` (`heeftTutorialVoltooid()`) is inmiddels wél opgelost (M20e): het verschijnt nu uitsluitend bij de tutorial-entry zelf (`campagne.id === undefined`), niet meer bij elke beschikbare campagne.
